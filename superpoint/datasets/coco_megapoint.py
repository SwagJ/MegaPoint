...
class CocoMegapoint(BaseDataset):
    default_config = {
        'labels': None,
    ...
    def _init_dataset(self, **config):
        ...
        for p in image_paths:
            semantic_path = os.fspath(base_path.parent) + '/semantics/' + os.fspath(p.stem) + '.jpg'
            #assert semantic_path.exists(), 'Image {} has no corresponding semantic {}'.format(p.stem, semantic)
            depth_path = os.fspath(base_path.parent) + '/depth/' + os.fspath(p.stem) + '.jpg'
